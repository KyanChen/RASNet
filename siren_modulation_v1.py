...
class Siren_Modulation(BaseModule):
    ...
    def init_weights(self):
        super(Siren_Modulation, self).init_weights()
    ...
    def forward(self, x, modulations):
        ...
        shift_modulations_split = torch.split(shift_modulations, list(self.modulation_size_dict.values()), dim=0)
        ...
        for i, layer_name in enumerate(self.layers):
            ...
            if shift_modulations_split[i] is not None:
                ...
                if self.is_residual and i != 0:
                    residual = layer(x, shift_modulations_split[i])
                    x = x + residual
                else:
